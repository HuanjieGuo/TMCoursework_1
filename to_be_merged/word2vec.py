...
def get_total_word_frequency(sentences):
    ...
    word_frequency = {word: count/total_count for word, count in word_count.items()}
...
def get_target_words(context, pos, max_window_size=5):
    ...
    right = pos+window_size+1 if pos+window_size < len(context) else len(context)
...
def train(vocabulary_size, embedding_dim, sentences, idx_word, batch_size=20, epochs=10):
